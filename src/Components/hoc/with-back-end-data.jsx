...
function withBackEndData(Component) {
  ...
  return class withBackEndData extends Component {
    state = {
      ...
      sort_by: JSON.parse(sessionStorage.getItem("sort"))
        ? JSON.parse(sessionStorage.getItem("sort"))
        : "normal",
      selected_items: {
        active_publishers: JSON.parse(sessionStorage.getItem("selected_items"))
          ? JSON.parse(sessionStorage.getItem("selected_items"))
              .active_publishers
          : [],
        active_subjects: JSON.parse(sessionStorage.getItem("selected_items"))
          ? JSON.parse(sessionStorage.getItem("selected_items")).active_subjects
          : [],
        active_grades: JSON.parse(sessionStorage.getItem("selected_items"))
          ? JSON.parse(sessionStorage.getItem("selected_items")).active_grades
          : [],
        active_course: JSON.parse(sessionStorage.getItem("selected_items"))
          ? JSON.parse(sessionStorage.getItem("selected_items")).active_course
          : [],
      },
    ...
    session_filter = () => {
      ...
      sessionStorage.setItem(
        "selected_items",
        JSON.stringify(this.state.selected_items)
      );
    };
    handle_destination = () => {
      ...
      const go_to = splited[splited.length - 2]
        ? "/bot-shop/" + splited[splited.length - 1]
        : "/bot-shop/shop";
      this.setState({ go_to });
    };
    ...
    handle_filter = () => {
      ...
      let { publishers, subjects, grades, courses } = this.state.filters;
      clicked_publishers.forEach((cp) => {
        filterd_by_pubs = filterd_by_pubs.concat(
          books.filter((b) => b.publisher === el_by_id(publishers, cp).name)
        );
      });
      ...
      clicked_courses.forEach((course) => {
        filtered_by_course = filtered_by_course.concat(
          books.filter((b) => b.course === el_by_id(courses, course).id)
        );
      });
      ...
      if (
        ...
        obj_id.forEach((id) => {
          filtered_book = filtered_book.concat(
    ...
    buy_coin = (amount) => {
      axios
        .get(
          `https://daryaftyar.ir/backend/api/buy_coin/id:${final_id}-amount:${amount}`
        )
        .then((res) => {
    ...
    handle_discount = (code) => {
      this.setState({ pause: true });
      axios
        .patch(`https://daryaftyar.ir/backend/api/cart_summary/${final_id}`, {
          dis_code: code,
        })
        .then((res) => {
