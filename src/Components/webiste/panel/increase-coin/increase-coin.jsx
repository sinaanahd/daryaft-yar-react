...
class IncreaseCoin extends Component {
    ...
    buy_coin = (amount) => {
        this.setState({ url: "" });
        axios
            .get(`https://daryaftyar.ir/backend/api/buy_coin/id:${this.props.user.user_id}-amount:${amount}`)
            .then(res => {
                    ...
                    <div className="buy-coin-page-wrapper">
                        <div className="coin-choose-wrapper">
                            <div className="options-wrapper">
                                <div 
                                    className="option"
                                    onClick={()=>{this.change_active(50)}}
                                >
                                    <span className={this.state.active === 50 ? "status active" : "status"}>
                                    </span>
                                    <span className="text">
                                        {50} سکه -
                                        {split_in_three(5000)}
                                ...
                                <div 
                                    className="option"
                                    onClick={()=>{this.change_active(150)}}
                                >
                                    <span className={this.state.active === 150 ? "status active" : "status"}>
                                    </span>
                                    <span className="text">
                                        {150} سکه -
                                        {split_in_three(9000)}
                                ...
                                <div 
                                    className="option"
                                    onClick={()=>{this.change_active(500)}}
                                >
                                    <span className={this.state.active === 500 ? "status active" : "status"}>
                                    </span>
                                    <span className="text">
                                        {500} سکه -
                                        {split_in_three(19000)}
                                        تومان
                                        (به‌صرفه‌ترین)
                                    </span>
                                </div>
                            ...
                            {
                                this.state.url !== "" ? 
                                    <a href={this.state.url} target="_blank" className="buy-coin-btn">
                                        خرید سکه
                                    </a>
                                    :
