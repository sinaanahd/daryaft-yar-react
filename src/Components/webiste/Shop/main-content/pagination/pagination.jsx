...
class Pagination extends Component {
    ...
    render() { 
        ...
        if (len_arr.length >= 5) {
            ...
            if (i !== 0 && i !== 1 && i !== len_arr.length-1 && i !== len_arr.length-2 ) {
                len_arr = [
                    len_arr[0],
                    len_arr[i - 1],
                    len_arr[i],
                ...
                <div className="pages-wrapper">
                    {
                        len_arr.length !== 0
                            ?
                            len_arr.map(i => typeof (i) === "number" ? <span key={i} className={i === active_page ? "page-number active" : "page-number"} onClick={()=>{change_active_page(i)}} >{i}</span> : <span key={Math.random()}>{ i }</span>)
                            :
                            <></>
                    }
                </div>
